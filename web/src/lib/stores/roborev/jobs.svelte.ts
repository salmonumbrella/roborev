...
export function createJobsStore(opts: JobsStoreOptions) {
  ...
  const fetchJobsAuthority = Effect.fn("RoborevJobs.fetchAuthority")(function* (
    ...
    const countQuery: ListJobsQuery = {
      ...query,
      limit: 0,
      omit_prompt: "true",
    };
    const countScope = JSON.stringify(countQuery);
    const countRequest = executeRoborevRequest(
      "count filtered Roborev jobs",
      (signal) =>
        client.GET("/api/jobs", {
    ...
    if (
      strictCount &&
      Option.isSome(result.count) &&
      result.count.value.error !== undefined
    ) {
      return yield* Effect.fail(
        RoborevResponseError.make({
    ...
    return {
      ...
      filteredStatusCounts: Option.filter(
        result.count,
        (count) => count.error === undefined,
      ).pipe(Option.map((count) => countJobsByStatus(count.data?.jobs ?? []))),
      countScope,
      ...
      yield* workflow.jobs(
        ...
        Effect.gen(function* () {
          ...
          yield* Effect.sync(() => {
            ...
            const fresh = (result.data?.jobs ?? []).filter(
  ...
  const rerunJobEffect = (id: number) =>
    Effect.gen(function* () {
      const workflow = yield* RoborevWorkflow;
      const query = buildQuery();
      const baselineRetryCount = yield* Ref.make<Option.Option<number>>(
        Option.none(),
      );
            ...
            Effect.catchTag("TransientTransportError", (cause) =>
              Effect.fail(
                RoborevResponseError.make({
                  ...
                  message:
                    "Failed to load Roborev rerun baseline before submission",
                  cause,
                }),
              ),
            ),
              ...
              Option.match({
                onNone: () =>
                  Effect.fail(
                    RoborevResponseError.make({
                      ...
                      message:
                        "Rerun baseline was unavailable during reconciliation",
                ...
                onSome: (retryCount) =>
                  reconcileJobMutation(
                    ...
                    acknowledged,
                    observedValue,
                    (job) => job.retry_count > retryCount,
                  ),
              }),
            ),
          ),
